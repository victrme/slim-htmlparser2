...
Deno.test('simple', function (t) {
	runTest(t.name, '<h1 class=test>adsf</h1>')
})

Deno.test('Template script tags', function (t) {
	runTest(t.name, '<p><script type="text/template"><h1>Heading1</h1></script></p>')
})

Deno.test('Lowercase tags', function (t) {
	runTest(t.name, '<H1 class=test>adsf</H1>', { lowerCaseTags: true })
})

Deno.test('CDATA', function (t) {
	runTest(t.name, '<tag><![CDATA[ asdf ><asdf></adsf><> fo]]></tag><![CD>', {
		xmlMode: true,
	})
})

Deno.test('CDATA (inside special)', function (t) {
	runTest(t.name, '<script>/*<![CDATA[*/ asdf ><asdf></adsf><> fo/*]]>*/</script>')
})

Deno.test('leading lt', function (t) {
	runTest(t.name, '>a>')
})

Deno.test('end slash: void element ending with />', function (t) {
...
Deno.test('end slash: non-void element ending with />', function (t) {
	runTest(t.name, '<xx / ><p>Hold the line.')
})

Deno.test('end slash: non-void element ending with />, recognizeSelfClosing=true', function (t) {
	runTest(t.name, '<xx / ><p>Hold the line.', { recognizeSelfClosing: true })
})

Deno.test('end slash: as part of attrib value of void element', function (t) {
	runTest(t.name, '<img src=gif.com/123/><p>Hold the line.')
})

Deno.test('end slash: as part of attrib value of non-void element', function (t) {
	runTest(t.name, '<a href=http://test.com/>Foo</a><p>Hold the line.')
})

Deno.test('Implicit close tags', function (t) {
	runTest(
		t.name,
...
Deno.test('attributes (no white space, no value, no quotes)', function (t) {
	runTest(t.name, '<button class="test0"title="test1" disabled value=test2>adsf</button>')
})

Deno.test('crazy attribute', function (t) {
	runTest(t.name, "<p < = '' FAIL>stuff</p><a")
})

Deno.test('Scripts creating other scripts', function (t) {
	runTest(t.name, "<p><script>var str = '<script></'+'script>';</script></p>")
})

Deno.test('Long comment ending', function (t) {
	runTest(t.name, "<meta id='before'><!-- text ---><meta id='after'>")
})

Deno.test('Implicit open p and br tags', function (t) {
	runTest(t.name, '<div>Hallo</p>World</br></ignore></div></p></br>')
})

Deno.test('lt followed by whitespace', function (t) {
	runTest(t.name, 'a < b')
})

Deno.test('double attribute', function (t) {
	runTest(t.name, '<h1 class=test class=boo></h1>')
})

Deno.test('numeric entities', function (t) {
	runTest(t.name, '&#x61;&#x62&#99;&#100&#x66g&#x;&#x68')
})

Deno.test('legacy entities', function (t) {
	runTest(t.name, '&AMPel&iacutee&ampeer;s&lter&sum')
})

Deno.test('named entities', function (t) {
	runTest(t.name, '&amp;el&lt;er&CounterClockwiseContourIntegral;foo&bar')
})

Deno.test('entity in attribute', function (t) {
	runTest(t.name, "<a href='http://example.com/p&#x61;#x61ge?param=value&param2&param3=&lt;val&; & &'>")
})

Deno.test('double brackets', function (t) {
	runTest(t.name, '<<princess-purpose>>testing</princess-purpose>')
})

Deno.test('legacy entities fail', function (t) {
	runTest(t.name, 'M&M')
})

Deno.test('Special special tags', function (t) {
	runTest(
		t.name,
...
Deno.test('Empty tag name', function (t) {
	runTest(t.name, '< ></ >')
})

Deno.test('Not quite closed', function (t) {
	runTest(t.name, '<foo /bar></foo bar>')
})

Deno.test('Entities in attributes', function (t) {
	runTest(t.name, '<foo bar=&amp; baz="&amp;" boo=\'&amp;\' noo=>')
})

Deno.test('CDATA in HTML', function (t) {
	runTest(t.name, '<![CDATA[ foo ]]>')
})

Deno.test('Comment edge-cases', function (t) {
	runTest(t.name, '<!-foo><!-- --- --><!--foo')
})

Deno.test('Comment false ending', function (t) {
	runTest(t.name, '<!-- a-b-> -->')
})

Deno.test('Scripts ending with <', function (t) {
	runTest(t.name, '<script><</script>')
})

Deno.test('tag names are not ASCII alpha', function (t) {
	runTest(t.name, '<12>text</12>')
})

Deno.test('entity in attribute (#276)', function (t) {
	runTest(t.name, '<img src="?&image_uri=1&&image;=2&image=3"/>?&image_uri=1&&image;=2&image=3')
})

Deno.test('entity in title (#592)', function (t) {
	runTest(t.name, '<title>the &quot;title&quot')
})

Deno.test('entity in title - decodeEntities=false (#592)', function (t) {
	runTest(t.name, '<title>the &quot;title&quot;', { decodeEntities: false })
})

Deno.test('</title> in <script> (#745)', function (t) {
	runTest(t.name, "<script>'</title>'</script>")
})

Deno.test('Trailing legacy entity', function (t) {
	runTest(t.name, '&timesbar;&timesbar')
})

Deno.test('Trailing numeric entity', function (t) {
	runTest(t.name, '&#53&#53')
})

Deno.test('Multi-byte entity', function (t) {
	runTest(t.name, '&NotGreaterFullEqual;')
})

Deno.test('Start & end indices from domhandler', function (t) {
	runTest(
		t.name,
...
Deno.test('Entity after <', function (t) {
	runTest(t.name, '<&amp;')
})

Deno.test('should handle errors', function (t) {
